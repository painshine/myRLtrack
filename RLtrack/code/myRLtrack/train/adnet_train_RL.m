...
%% add test
test1 = net.params(13).value;
test2 = max(test1,[],4);
net.params(13).value = (test2);
...
net.params(13).value = cat(3, net.params(13).value, zeros(1,1,nvec,1));
...
%             this_actions = zeros(num_show_actions, 1);
        ...
        fprintf('time: %04.2f, epoch: %02d/%02d, vid: %05d/%05d, loss: %03.4f \n', ...
            toc(tic_train), epoch, opts.numEpoch, vid_idx, numel(video_names), curr_loss);
...
net.params(net.getParamIndex(fc_layer_score.params{1})).value(:,:,1:512,:) = 0.01 * randn(1,1,512,1,'single');
net.params(net.getParamIndex(fc_layer_score.params{2})).value = zeros(1, 1, 'single');
